...
export const Services: React.FC = () => {
  return (
    <div id="services" className="py-32 relative z-10 bg-[#030014]">
      {/* Background Accents */}
      <div className="absolute right-0 top-1/3 w-96 h-96 bg-fuchsia-600/10 blur-[100px] rounded-full pointer-events-none"></div>
      <div className="absolute left-0 bottom-1/3 w-96 h-96 bg-indigo-600/10 blur-[100px] rounded-full pointer-events-none"></div>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        <div className="mb-20 md:text-center max-w-3xl mx-auto">
           <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 text-indigo-400 text-xs font-bold uppercase tracking-wider mb-6 backdrop-blur-sm">
               <Command className="w-3 h-3" />
               <span>Capabilities</span>
           </div>
           <h3 className="text-4xl md:text-5xl font-bold text-white tracking-tight mb-6">
             Full-stack engineering for <br/>
             <span className="text-indigo-400">the AI era.</span>
           </h3>
...
const ServiceCard: React.FC<{ service: any; index: number }> = ({ service, index }) => {
    ...
    return (
        <div 
            ...
            className={`
                group relative rounded-3xl border border-white/10 bg-white/5 backdrop-blur-sm overflow-hidden hover:bg-white/[0.08] transition-all duration-500 hover:border-white/20 hover:shadow-2xl hover:shadow-indigo-500/10
        ...
        >
            ...
            <div className={`h-full flex ${isLarge ? 'flex-col md:flex-row' : 'flex-col'}`}>
                ...
                <div className={`p-8 flex flex-col justify-between relative z-10 ${isLarge ? 'md:w-1/2' : 'w-full flex-1'}`}>
                    <div>
                        <div className="w-12 h-12 rounded-xl bg-white/5 flex items-center justify-center mb-6 border border-white/10 group-hover:scale-110 group-hover:bg-indigo-500/20 group-hover:border-indigo-500/50 transition-all duration-300 shadow-lg shadow-black/20">
                            {React.cloneElement(IconMap[service.icon] as React.ReactElement<any>, { 
                                className: "w-6 h-6 text-slate-300 group-hover:text-indigo-400 transition-colors" 
                            })}
                        </div>
                        <h3 className="text-2xl font-bold text-white mb-3 group-hover:text-indigo-200 transition-colors">{service.title}</h3>
                        <p className="text-slate-400 leading-relaxed text-sm font-light">
                            {service.description}
                        </p>
                    </div>
                    <div className="mt-6">
                        <span className="inline-flex items-center gap-2 text-xs font-bold text-indigo-400 uppercase tracking-wider opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300">
